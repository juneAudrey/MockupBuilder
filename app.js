...
function snapshot(){return JSON.parse(JSON.stringify({comps,uid}));}
...
function undo(){
  ...
  comps=s.comps; uid=s.uid; selectSingle(null);
...
<script type="application/json" id="__mb_src__">${
  JSON.stringify({v:1,comps,cw,ch,skin:document.body.classList.contains('skin-classic')?'fat':'thin'}).replace(/<\//g,'<\\u002f')
}<\/script>
</body></html>`;
  return out;
}
...
function openConvert(){
  ...
  document.getElementById('jsonPrompt').value=currentJsonPrompt('copy');
  cvSetStep(1);
  clearStatus();
...
// kind='url' → Claude 열기(새 창 URL 파라미터, 인코딩 팽창 때문에 짧아야 함)
...
function copyPrompt(){
  navigator.clipboard&&navigator.clipboard.writeText(currentJsonPrompt('copy')).then(
...
function mbBuildSaveData(){
  ...
  return {v:1, comps, cw, ch, skin:document.body.classList.contains('skin-classic')?'fat':'thin'};
}
...
function mbCloudSharedBaseQuery(){
  let q=`/mockups?is_public=eq.true&mode=eq.${mbCurrentMode()}&select=id,title,tags,created_at,owner_id,thumbnail,open_count`;
...
function mbCloudRenderMine(){
  ...
  const toolbar=`<div class="cl-toolbar">
    ...
    <select onchange="mbCloudMineSort(this.value)" title="정렬 기준">
...
async function mbCloudDoOpen(){
  ...
  try{
    const rows=await mbRestFetch(`/mockups?id=eq.${mbCloud.selectedId}&select=data,title`);
...
function mbCloudRenderShared(){
  ...
  const toolbar=`<div class="cl-toolbar">
    ...
    <select onchange="mbCloudSharedSort(this.value)">
    ...
    mbCloud.sharedItems.forEach(it=>{
      ...
      rows+=`<div class="cl-explorer-row ${sel?'sel':''}" onclick="mbCloudSharedSelect('${it.id}')" ondblclick="mbCloudSharedOpen('${it.id}')">
        ...
        <div class="cl-explorer-meta">
          <span class="cl-name-text">${esc(it.title)}</span>
    ...
    mbCloud.sharedItems.forEach(it=>{
      ...
      cards+=`<div class="cl-card ${sel?'sel':''}" onclick="mbCloudSharedSelect('${it.id}')" ondblclick="mbCloudSharedOpen('${it.id}')">
        ...
        <div class="cl-card-body">
          <div class="cl-card-title">${esc(it.title)}</div>
...
async function mbCloudSharedOpen(id){
  if(!id) return;
  try{
    const rows=await mbRestFetch(`/mockups?id=eq.${id}&select=data,title`);
    const row=rows&&rows[0]; if(!row) throw new Error('파일을 찾을 수 없습니다.');
    mbCloudApplyData(row.data);
    closeCloud();
