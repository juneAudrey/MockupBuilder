...
async function openDraftList(){
  ...
  let rows=[];
  try{ rows=await mbRestFetch(`/mockup_drafts?owner_id=eq.${s.id}&mode=eq.${mbCurrentMode()}&order=updated_at.desc&select=id,title,updated_at`)||[]; }
  ...
  body.innerHTML=rows.map(r=>`
    <div class="draft-row">
      <div class="draft-row-main" onclick="mbDraftOpen('${r.id}')">
        <div class="draft-row-title">${esc(r.title||'(제목 없는 화면)')}</div>
...
async function mbDraftOpen(id){
  if(!confirm('이 임시 작업을 불러오면 지금 화면의 작업 내용은 사라집니다.\n계속하시겠습니까?')) return;
  try{
    ...
    // 이 항목이 원래 쓰던 세션 키를 이어받는다 - 안 이어받으면 이어서 수정할 때 지금 열기 전에
    // 하고 있던(전혀 다른) 작업의 임시 저장 행에 이 내용을 덮어써 버리고, 방금 연 이 항목은 DB에
    // 그대로 남아 목록에 중복으로 계속 보이게 된다.
    if(row.session_key) mbSetDraftSessionKey(row.session_key);
  }catch(e){ alert('열지 못했습니다.\n\n'+e.message); }
}
async function mbDraftDelete(id){
  if(!confirm('이 임시 작업을 목록에서 삭제하시겠습니까?')) return;
...
async function mbAdminSetDraftLimit(userId,inputEl){
  ...
  if(isNaN(v)||v<0){ alert('0 이상의 숫자를 입력해 주세요.'); inputEl.value=(mbAdmin.draftLimits[userId]!=null)?mbAdmin.draftLimits[userId]:30; return; }
...
async function mbAdminResetPassword(userId,username,btnEl){
  if(!confirm(`'${username}' 님의 비밀번호를 초기화하시겠습니까?\n초기화하면 비밀번호 없이(빈 칸으로) 로그인할 수 있게 됩니다.`)) return;
  ...
  try{
    ...
    alert(`'${username}' 님의 비밀번호를 초기화했습니다.`);
  }catch(e){ alert('초기화하지 못했습니다.\n\n'+e.message); }
...
async function mbCloudDoOpen(){
  ...
  try{
    ...
    mbCloudApplyData(row.data);
    mbResetDraftSessionKey(); // 다른 파일을 열었으므로 이건 별개의 새 작업 - 새 세션 키로 시작한다.
    closeCloud();
  }catch(e){ alert('열지 못했습니다.\n\n'+e.message); }
}
