  ...
  switch(c.type){
    ...
    case 'grid':{
      ...
      const colDragAttrs=ci=>{
        ...
        const clsAll=[cls,stCls].filter(Boolean).join(' ');
      ...
      // 컬럼별 변경상태(추가/변경/삭제/이동) 헤더 표시도 같은 이유로 "상위헤더 없는 컬럼은 2행을
      // 그대로 span"하는 트릭을 쓴다 - 라벨 위에 살짝 겹치는 코너 태그 대신, 상태가 있는 컬럼만
      // 1행에 작은 색상 텍스트를 넣는 방식. .ax-grid 자체가 overflow:hidden이라 헤더 밖으로
      // 튀어나오는 절대배치 라벨은 그대로 잘려버리므로, 실제 헤더 높이 안에 들어가야 한다.
      const hasColStatus=colStatusArr.some(s=>s&&s!=='base');
      const hasTopRow=hasGroups||hasColStatus;
      ...
      if(hasTopRow){
        ...
        for(let ci=0;ci<cols.length;ci++){
          ...
          if(g){
            ...
            // 상위헤더가 없는데 상태만 있는 컬럼: 1행에 작은 색상 태그, 2행에 원래 컬럼명.
            headParts.push(`<span class="gcol-difftag-row gcol-diffst-${cst}" style="grid-column:${ci+1+off};grid-row:1;">${DIFF_LABEL[cst]}</span>`);
            headParts.push(`<span${colDragAttrs(ci)} style="grid-column:${ci+1+off};grid-row:2;text-align:${align};position:relative;">${mark}${esc(cols[ci].trim())}${resizeHandle(ci)}</span>`);
          }else{
    ...
    case 'searchbar':{
      ...
      const fields=cells.map(cell=>{
        ...
        if(f.type==='empty'){
          return `<div class="sfield sfield-empty${diffBoxCls(f.status)}" style="grid-column:span ${cell.units};"${diffBoxAttr(f.status)}></div>`;
        }
        return `<div class="sfield${f.type==='radio'?' radio':''}${diffBoxCls(f.status)}" style="grid-column:span ${cell.units};"${diffBoxAttr(f.status)}>${sfFieldInner(f,fi,exp,c.id)}</div>`;
      }).join('');
...
function sfHalfBox(f,fi,exp,cid){
  if(f.type==='empty') return `<div class="sfield sfield-half sfield-empty${diffBoxCls(f.status)}"${diffBoxAttr(f.status)}></div>`;
  return `<div class="sfield sfield-half${f.type==='radio'?' radio':''}${diffBoxCls(f.status)}"${diffBoxAttr(f.status)}>${sfFieldInner(f,fi,exp,cid)}</div>`;
}
// palette for charts
const CHART_PAL={
...
function upd(k,v){
  ...
  if(k==='showLabel'||k==='labelPos'||k==='collapsed'||k==='perRow'||k==='xscroll'||k==='style'||k==='diffStatus'){render();}else{drawCanvas();}
}
...
function moveGridColumn(c,from,to){
  ...
  move(c.colWidths);move(c.colDateVals);move(c.colDateBlank);move(c.colDateSpec);move(c.colStatus);
  return true;
}
function gcDrop(e,i){
  e.preventDefault();
...
(function(){
  ...
  function applyColumnWrapMode(){
    var reverts=overflowingGrids().map(function(x){
      ...
      var compactH=Math.ceil(rows[0].getBoundingClientRect().bottom-item.getBoundingClientRect().top)+2;
